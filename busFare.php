<?php
  ...
  { 
    ...
    {
      foreach ($seats as $seat) {
      $sql = "insert into seat_info (seat_num, bus_id, status) values ('$seat', '$busId', 'checked')";
      if(!mysql_query($sql))
      {
    ...
    $sql = "update bus_trips set avai_seat = avai_seat-$count where bus_id=$busId;";
     ...
     //*****************//
      ...
      $busId =  $_SESSION["bus_id"];
      $sql1 = "insert into ticket_invoice (user_name, email, phone, gender,bus_id,vehicle_name, pay_method, amount, seats, category, route, jdate, jtime) values ('$fname', '$email','$phone','$gender','$busId','$v_name','$payment','$amount','$seatNum','$category','$route','$jdate','$jtime');";
      if(!mysql_query($sql1))
        echo mysql_error();
      else
        {
