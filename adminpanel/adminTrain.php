  ...
  $err = "";
   if($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_POST['submit']))
      {    
        ...
        $sql ="INSERT INTO train_trips (tourdate, t_name, deptfrom, goingto, category, dept_time, arri_time, fare_per_seat, avai_seat) VALUES ('$date', '$t_name', '$dept_from', '$going_to', '$category', '$dept_time', '$arri_time', '$fare', 40)";
        if(!mysql_query($sql))
          {
 ...
 <header>
      ...
      <nav>
        <ul>
          ...
          <li><a href="adminTrain.php" class="selected">TrainInfo</a></li>
...
<center>
<h3 style="color: #2F4F4F;"> Admin Panel || Trains </h3>
...
<form method="POST">
  ...
  <td onclick="document.location = 'trainTripDelete.php?id=<?php echo urlencode($row['train_id']);?>'" style="background-color: coral; color: white; font-size: 20px; cursor:hand;">Delete</td>
      </tr>                
<?php
   }
?>
    ...
    <table border="1" style="border-collapse: collapse; width:100%;">
      ...
      <tr>
		...
		<select name="seat_category" required>
				    ...
				    <option value="FirstClass AC">FirstClass AC</option>
				    <option value="FirstClass Chair">FirstClass Chair</option>
...
<!-- All Ticket Print -->
  <div>
    ...
    <form action="printAllTrainTicket.php" method="POST">
           <?php        
              $sql = "SELECT user_name, jdate, vehicle_name, route, category, jtime, seats, amount, pay_method, phone, train_id, ticket_id FROM ticket_invoice WHERE ticket_invoice.flight_id IS NULL AND ticket_invoice.bus_id IS NULL UNION SELECT name, jdate, vehicle_name, route, category, jtime, seats, amount, pay_method, phone, train_id, ticket_id FROM user_info, user_ticket_info WHERE user_info.user_id = user_ticket_info.user_id AND user_ticket_info.flight_id IS NULL AND user_ticket_info.bus_id IS NULL";
              $result = mysql_query($sql);
              $i = 0;        
            ?>
    <table border="1" style="border-collapse: collapse; width:100%;">
