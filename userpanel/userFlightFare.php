  ...
  { 
    ...
    $sql = "update flight_trips set avai_seat = avai_seat-$count where flight_id=$flightId;"; // 0 is allowed but not (-) value
     ...
     //*****************//
      ...
      $sql1 = "insert into user_ticket_info (user_id,flight_id,vehicle_name, pay_method, amount, seats, category, route, jdate, jtime) values ('$getUID','$flightId','$v_name','$payment','$amount','$seatNum','$category','$route','$jdate','$jtime');";
      if(!mysql_query($sql1))
        echo mysql_error();
      else
        {
          ...
          //****************//
         header('Location: userFlightTicket.php');
         exit(); 
       }
    }
    else
